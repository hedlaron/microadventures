...
from sqlalchemy import Integer, String, DateTime, Text, JSON, ForeignKey, Boolean
...
class Adventure(Base):
    ...
    recommendations: Mapped[dict] = mapped_column(JSON, nullable=False)  # Store recommendations as JSON
    ...
    share_token: Mapped[str] = mapped_column(String(36), nullable=True, unique=True)  # UUID for public sharing
...
class AdventureQuota(Base):
    ...
    last_reset_date: Mapped[DateTime] = mapped_column(DateTime, default=datetime.now(timezone.utc), nullable=False)
