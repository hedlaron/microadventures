  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 429) {
        message =
          "You have reached your daily adventure limit. Please try again tomorrow.";
      } else if (error.response.data && error.response.data.detail) {
        message = error.response.data.detail;
      }
    }
...
const shareAdventure = async (adventureId, makePublic) => {
  try {
    ...
    if (error.response && error.response.status === 401) {
      ...
      error.userMessage =
        "Adventure not found or you don't have permission to share it.";
    }
    throw error;
  }
};
...
const getSharedAdventure = async (shareToken) => {
  try {
    const response = await axios.get(
      `${API_URL}/adventures/shared/${shareToken}`,
    );
    return response.data;
  } catch (error) {
    ...
    if (error.response && error.response.status === 404) {
      error.userMessage =
        "This shared adventure is no longer available or was made private.";
    }
    throw error;
  }
};
