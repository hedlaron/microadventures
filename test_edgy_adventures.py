...
def test_adventure_generation():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"🎯 Generating edgy adventure for Brooklyn...")
            start_time = time.time()
            
            response = requests.post(
                f"{base_url}/api/adventures/generate",
                json=adventure_request,
                headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Adventure generated successfully!")
                print(f"📍 Title: {adventure['title']}")
                print(f"📝 Description: {adventure['description'][:200]}...")
                print(f"🖼️  Image URL: {'✅ Present' if adventure.get('image_url') else '❌ Missing'}")
                ...
                history_response = requests.get(
                    f"{base_url}/api/adventures/my-history",
                    headers=headers
                )
                ...
                if history_response.status_code == 200:
                    ...
                    print(f"❌ History failed: {history_response.status_code} - {history_response.text}")
